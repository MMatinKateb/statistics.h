...
template <class T>
// maximum value
double max(T arr[], int size_of_array) {
	double max = 0;
	for (int i=0; i<size_of_array; i++)
		if(max < arr[i])
			max = arr[i];
...
template <class T>
// minimum value
double min(T arr[], int size_of_array) {
	double min = max(arr, size_of_array);
	for (int i=0; i<size_of_array; i++)
		if(min > arr[i])
			min = arr[i];
	return min;
...
template <class T>
// range
double range(T arr[], int size_of_array) { // maximum - minimum
    double res = max(arr, size_of_array) - min(arr, size_of_array);
    return res;
}
